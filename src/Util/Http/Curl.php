...
namespace zxf\Util\Http;

use Closure;
use CURLFile;
use Exception;
use zxf\Util\Data\Collection;
...
class Curl
{
    ...
    // 请求前回调函数
    private ?Closure $beforeFunc = null;

    // 请求后回调函数
    private ?Closure $afterFunc = null;
    ...
    private function parserResponse(): static
    {
        ...
        if (str_starts_with($body, 'callback(')) {
            ...
            if (! $this->isJson($body)) {
                // 判断 等号出现次数
                $countEqStr = substr_count($body, '=', 0);
                if ($countEqStr > 0) {
                    parse_str($body, $body);
                }
            }
